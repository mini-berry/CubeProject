...
#define MPU6050																	//如果加入keil的魔术棒中的宏定义, 这里就不用宏定义了,下面会对应开启它的相关结构体、寄存器等

#define STM32F103xx													//这个在keil的魔术棒中已经宏定义了, 不同的型号生成不同的宏定义, 

#include "mpu6050.h"														//mpu6050的底层驱动, 符合DMP下面的 i2c_write,i2c_read 两函数
#include "stm32f1xx.h"													//芯片型号的头文件
#include "i2c.h"																//HAL库用到的i2c接口
#include "stm32f1xx_hal.h"											//HAL库没有为专门延时单独写个库, Hal_Delay 在stm32f4xx_hal.h中

#define i2c_write   dmp_i2c_write								//i2c写
#define i2c_read    dmp_i2c_read								//i2c读
#define delay_ms    my_hal_delay								//延时
#define get_ms      f1_get_tick_ms_cnt					//时间戳
...
//    if (int_param)																																			//把mpu_init的形参给去掉
//        reg_int_cb(int_param);																													//注释这里的入口参数,				修改了
